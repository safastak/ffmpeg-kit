 ...
 * - Scheduler-based transcode loop adapted for ffmpeg-kit
 * - forward_report and report_callback updated for new print_report signature
 * - __thread added to new variables (decoders, nb_decoders, vstats_file, nb_output_dumped as atomic)
 *
 * 07.2023
 * --------------------------------------------------------
...
static void forward_report(uint64_t frame_number, float fps, float quality, int64_t total_size, int secs, int us, double bitrate, double speed)
{
    // FORWARD DATA
    if (report_callback != NULL) {
        report_callback(frame_number, fps, quality, total_size, ((double)secs*1000) + ((double)us)/1000, bitrate, speed);
    }
}
...
static void print_report(int is_last_report, int64_t timer_start, int64_t cur_time, int64_t pts)
{
    ...
    for (OutputStream *ost = ost_iter(NULL); ost; ost = ost_iter(ost)) {
        q = ost->enc ? atomic_load(&ost->quality) / (float) FF_QP2LAMBDA : -1;
...
static int transcode(Scheduler *sch)
{
    ...
    while (!sch_wait(sch, stats_period, &transcode_ts) && !received_sigterm && !cancelRequested(globalSessionId)) {
...
int ffmpeg_execute(int argc, char **argv)
{
    ...
    extern void init_ffmpeg_options(void);
    init_ffmpeg_options();
...
finish:
        ...
        main_ffmpeg_return_code = (received_nb_signals || cancelRequested(globalSessionId)) ? 255 : ret;
