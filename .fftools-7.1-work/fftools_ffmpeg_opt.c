...
static const char *const alt_bsf[] __attribute__((unused)) = { "absf", "vbsf", NULL };
...
void init_ffmpeg_options(void) {
    ...
    // Count entries to determine size
    int count = 0;
    while (options[count].name) count++;
    count++; // include the NULL terminator entry
    size_t sz = count * sizeof(OptionDef);
    // Copy to heap so it outlives this function
    OptionDef *opts = av_malloc(sz);
    if (opts) memcpy(opts, options, sz);
    ffmpeg_options = opts;
}
